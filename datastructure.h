...
struct VNEmbedding {
  ...
  std::unique_ptr<std::map<std::pair<int,int>, std::vector<std::pair<int,int>>>> edge_map;
  long cost;
};
...
struct VNRParameters {
  ...
  std::string GetDebugString() {
    return "alpha = " + std::to_string(alpha) + ", beta = " + std::to_string(beta)
            + ", gamma = " + std::to_string(gamma) + ", util_threshold = " + 
